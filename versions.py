...
class VersionManager:
    ...
    def load_versions(self):
        """Carregar todas as versões do JSON."""
        for item in self.tree_versions.get_children():
            self.tree_versions.delete(item)

        try:
            with open(self.json_file, "r", encoding="utf-8") as f:
                data = json.load(f).get("versions", {})
                for version in sorted(data.keys()):
                    self.tree_versions.insert("", "end", values=(version,))
        except Exception as e:
            messagebox.showerror("Error", f"Failed to load versions: {str(e)}")

    def create_colored_table(self, parent, data=None):
        """Criar uma tabela colorida para RT, LT e HT com alternância de cores."""
        frame_table = ttk.Frame(parent)
        frame_table.pack(fill="both", expand=True, padx=10, pady=10)

        labels = ["pk10", "pk25", "pk50", "pk75", "pk90", "pkmax"]
        temperatures = ["RT", "LT", "HT"]
        categories = ["minimum", "baseline", "maximum"]
        colors = {
            "RT": ["#DFFFD6", "#B8E2A7", "#8CC882"],  # Verde pastel claro → escuro
            "LT": ["#D6ECFF", "#A7D0E2", "#82B8CC"],  # Azul pastel claro → escuro
            "HT": ["#FFD6D6", "#E2A7A7", "#C88282"],  # Vermelho pastel claro → escuro
        }

        entries = {}

        # Criar cabeçalhos com fundo cinza claro
        ttk.Label(frame_table, text="Temp", background="#F0F0F0").grid(
            row=0, column=0, sticky="ew"
        )
        for j, label in enumerate(labels):
            ttk.Label(frame_table, text=label, background="#F0F0F0").grid(
        ...
        for temp in temperatures:
            for idx, category in enumerate(categories):
                color = colors[temp][idx]  # Alterna cor para cada linha

                # Criar um frame de fundo colorido para manter a estética
                row_frame = tk.Frame(frame_table, background=color)
                row_frame.grid(
                    row=row_index, column=0, columnspan=len(labels) + 1, sticky="ew"
                )

                # Nome da linha com fundo colorido
                ttk.Label(
                    row_frame, text=f"{temp} - {category}", background=color
                ).grid(row=0, column=0, sticky="ew")

                entries[(temp, category)] = []
                for j, label in enumerate(labels):
                    value = (
                        data.get(temp, {}).get(category, {}).get(label, 0.0)
                        if data
                        else 0.0
                    )
    ...
    def add_version(self):
        """Adicionar uma nova versão utilizando uma tabela colorida."""
        add_window = tk.Toplevel(self.root)
        add_window.title("Add New Version")
        ...
        )  # Acesse `frame_table` para garantir que seja utilizado

        def save_new_version():
            new_version_data = {
                temp: {cat: {} for cat in ["minimum", "baseline", "maximum"]}
                for temp in ["RT", "LT", "HT"]
            }
            for temp in ["RT", "LT", "HT"]:
                for category in ["minimum", "baseline", "maximum"]:
                    for j, label in enumerate(
                        ["pk10", "pk25", "pk50", "pk75", "pk90", "pkmax"]
                    ):
                        value = entries[(temp, category)][j].get()
                        try:
                            new_version_data[temp][category][label] = float(value)
                        except ValueError:
                            new_version_data[temp][category][label] = 0.0

            with open(self.json_file, "r+", encoding="utf-8") as f:
                data = json.load(f)
                if "versions" not in data:
                    data["versions"] = {}
                data["versions"][version_name] = new_version_data
                f.seek(0)
    ...
    def edit_version(self):
        ...
        # Garantir foco na nova janela
        edit_window.transient(self.root)
        edit_window.grab_set()
        edit_window.focus_set()

        with open(self.json_file, "r", encoding="utf-8") as f:
            data = json.load(f).get("versions", {}).get(selected_version, {})

        # Criar tabela estilizada com cores pastéis
            ...
            for temp in ["RT", "LT", "HT"]:
                for category in ["minimum", "baseline", "maximum"]:
                    for j, label in enumerate(
                        ...
                        value = entries[(temp, category)][j].get()
                        try:
                            updated_data[temp][category][label] = float(value)
                        except ValueError:
                            updated_data[temp][category][label] = 0.0

            with open(self.json_file, "r+", encoding="utf-8") as f:
                full_data = json.load(f)
                full_data["versions"][selected_version] = updated_data
                f.seek(0)
                json.dump(full_data, f, ensure_ascii=False, indent=2)
                f.truncate()

            messagebox.showinfo("Success", "Version updated successfully!")
            edit_window.destroy()
            self.load_versions()
    ...
    def delete_version(self):
        ...
        if messagebox.askyesno(
            ...
            try:
                with open(self.json_file, "r+", encoding="utf-8") as f:
                    data = json.load(f)
                    del data["versions"][selected_version]
                    f.seek(0)
                    json.dump(data, f, ensure_ascii=False, indent=2)
                    f.truncate()
